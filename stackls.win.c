...
_noreturn_inline void fnErrorExit(LPTSTR pszFunction)
{
    PVOID pMsgBuf;
    PVOID pDisplayBuf;
    ...
    StringCchPrintf((LPTSTR)pDisplayBuf, LocalSize(pDisplayBuf) / sizeof(TCHAR), TEXT("%s failed with error %d: %s"),
...
{
    CTX_StackIsAtBottom = !(StackWalk64(MACHINE_TYPE, *CTX_ProcessHandle, NO_THREADS, CTX_CurrentStackFrame,
                                        CTX_MachineContext, NULL, SymFunctionTableAccess64, SymGetModuleBase64, NULL));
}

_hotbed_inline void fnStacklsExtractSymbolFromFrame(PSTACKLSCTX pStraceCtx)
{
    winerror_CHECK(
        SymFromAddr(*CTX_ProcessHandle, CTX_CurrentStackFrame->AddrPC.Offset, CTX_Displacement, CTX_CurrentSymbol),
        SymFromAddr);
    winerror_CHECK(UnDecorateSymbolName(CTX_CurrentSymbol->Name, (PSTR)CTX_LastSymblStr, MAX_SYM_NAME,
                                        &CTX_Displacement, UNDNAME_COMPLETE),
